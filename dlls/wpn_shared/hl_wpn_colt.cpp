...
LINK_WEAPON_TO_CLASS( weapon_colt, CColt);
...
void CColt::Precache(void)
{
	...
	PRECACHE_SOUND("weapons/colt/coltf1.wav");

	m_usFireColt = PRECACHE_EVENT(1, "events/colt1.sc");
}
...
BOOL CColt::Deploy()
{
	return DefaultDeploy("models/v_colt.mdl", "models/p_9mmhandgun.mdl", GUN_RAISE, "onehanded");
}
...
void CColt::PrimaryAttack(void)
{
	...
	PLAYBACK_EVENT_FULL(flags, m_pPlayer->edict(), m_usFireColt, 0.0, (float*)&g_vecZero, (float*)&g_vecZero, vecDir.x, vecDir.y, 0, 0, 0, 0);
...
void CColt::Reload(void)
{
	...
	DefaultReload(8, GUN_RELOAD1, 1.95f);
